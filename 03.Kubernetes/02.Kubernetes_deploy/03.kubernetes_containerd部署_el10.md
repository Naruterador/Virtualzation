...
##### 1.1 检查操作系统的版本
- 本次安装kubernetes集群使用的版本是Rocky Linux release 10
```shell
[root@kube-master1 ~]# cat /etc/redhat-release
Rocky Linux release 10 (Green Obsidian)
```

##### 1.2 DNS域名解析
- 为集群内主机配置好DNS解析
```shell
kube-master1.skills.com 192.168.101.41
kube-node1.skills.com   192.168.101.43
kube-node2.skills.com   192.168.101.44
```
...
- 安装包版本为:
```shell 
containerd.io-2.2.1-1.el10.x86_64.rpm
```
...
```
 ...
 - 编辑/etc/containerd/config.toml配置文件
   - 将config.toml文件中的SystemdCgroup = false,改为 true,在config.toml文件的第109行
   - 修改sandbox_image = "k8s.gcr.io/pause:x.x"，将pause版本改为当前使用的版本
   - 启动和设置开机自启containerd
```shell
[root@kube-master1 ~]# systemctl restart containerd
[root@kube-master1 ~]# systemctl enable containerd
```
...
```
  ...
  - 架设好yum仓库后使用如下命令安装:
```shell
[root@kube-master1 ~]# dnf install kube*
```
...
```
   ...
   - 设置kubelet开机自启
```shell
[root@kube-master1 ~]# systemctl enable kubelet
```
...
- 使用如下命令查看需要的容器:
`[root@kube-master1 ~]# kubeadm config images list`

##### 1.10 准备集群镜像（镜像名称根据实际情况调整）
- 在安装kubernetes集群之前，必须要提前准备好集群需要的镜像，所需镜像可以通过下面命令查看
- 我们这里采用离线方式安装，事先会讲集群的容器全部下载好，并且导入到所有节点
- 使用如下命令将集群容器导入containerd
```shell
ctr -n k8s.io i import coredns.tar --platform linux/amd64
ctr -n k8s.io i import etcd.tar --platform linux/amd64
ctr -n k8s.io i import kube-apiserver.tar --platform linux/amd64
ctr -n k8s.io i import kube-proxy.tar --platform linux/amd64
ctr -n k8s.io i import kube-scheduler.tar --platform linux/amd64
ctr -n k8s.io i import kube-controller-manager.tar --platform linux/amd64
ctr -n k8s.io i import pause.tar --platform linux/amd64
ctr -n k8s.io i import cni.tar --platform linux/amd64  #pod内部的网络插件
ctr -n k8s.io i import kube-controller.tar --platform linux/amd64 #pod内部的网络插件
ctr -n k8s.io i import node.tar --platform linux/amd64 #pod内部的网络插件
```
- --platform 参数为镜像架构 linux/amd64 和 linux/arm64 分别带料x86和arm架构
- 查看导入容器
`ctr -n k8s.io i list`
...
kubeadm init 
#master节点地址
--apiserver-advertise-address=192.168.101.41  
#kubernetes版本              
...
#使用containerd作为集群镜像启动器                   
--cri-socket=/run/containerd/containerd.sock        
#指定k8s容器下载路径，默认为google,这里指定为了阿里云（可选配置，如果容器是从阿里云下载）
...
- 以下命令将node节点加入群集
```shell
kubeadm join 192.168.101.41:6443 --token god06n.fow0yn46h004djgs \
...
kube-master1.skills.com   NotReady   control-plane,master   4m3s   v1.35.0
kube-node1.skills.com    NotReady   <none>                 61s    v1.35.0
kube-node2.skills.com    NotReady   <none>                 40s    v1.35.0
```
**这里我们看到所有的几点都是NotReady状态，这是因为没有安装网络插件**

##### 1.12 安装网络插件，只在master节点操作即可
- 这里我们用的网络插件是比较常见的calio，当然还有别的网络插件也可以使用
- 我们前面已经将fcalio的容器导入到containerd中了，那么现在我们只需要使用calio的yaml文件部署一下插件即可，calio.yaml文件可以去官网直接下载
 - 执行以下指令部署calico网络插件：
 ```shell
...
[root@kube-master1]# kubectl get nodes
NAME                     STATUS   ROLES                  AGE     VERSION
kube-master1.skills.com   Ready    control-plane,master   8m55s   v1.35.0
kube-node1.skills.com    Ready    <none>                 5m53s   v1.35.0
kube-node2.skills.com    Ready    <none>                 5m32s   v1.35.0
```
**可以看到已经显示为Ready了，到这里我们的kubernetes集群就搭建好了**

##### 1.13集群测试

- 将nginx容器导入到container中
`ctr -n k8s.io i import nginx.tar`

- 修改容器名称
`ctr -n k8s.io i tag docker.io/library/nginx:latest docker.io/library/nginx:1.1`

- 由于这里默认nginx的名称tag为latest，默认container会从网上拉取镜像而不是读取本地镜像,所以我们需要修改容器名称
- 创建一个nginx服务
```shell
kubectl create deployment nginx  --image=docker.io/library/nginx:1.1 
```
